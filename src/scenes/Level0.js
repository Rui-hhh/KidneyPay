class Level0 extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.tilemapTiledJSON('level0Map', './assets/Level0.json');
        this.load.spritesheet("level0_sheet", './assets/tile_set.png', {
            frameWidth: 32,
            frameHeight: 32
        });
    }
    ...
    create() {
        // tilemap settings
        const map = this.add.tilemap('level0Map');

        let bottomHeight = map.heightInPixels;
        console.log(bottomHeight);

        const tileset = map.addTilesetImage('tile_set', 'level0_sheet');
    ...
    dropBridge(leverSpawn, groundLayer, bridgeSpawn, player) {
        ...
        this.physics.add.collider(muscleMan, this.bridge);
    ...
    dollExplode(){
        this.explode.play();
        // trap hit effect
        let particles = this.add.particles('fragmentDoll');
