class Level2 extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.tilemapTiledJSON('level2Map', './assets/Level-2.json');
        this.load.spritesheet("level2_sheet", './assets/tile_set.png', {
            frameWidth: 32,
            frameHeight: 32
        });
    }
    ...
    create() {
        // tilemap settings
        const map = this.add.tilemap('level2Map');

        let bottomHeight = map.heightInPixels;
        console.log(bottomHeight);

        const tileset = map.addTilesetImage('tile_set', 'level2_sheet');
